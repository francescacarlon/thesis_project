"""
HTML Generator for Tailored Text Benchmark

This script creates human-readable HTML pages from AI-generated tailored texts,
used in a benchmark comparing paraphrased explanations for different target groups
(e.g., Computer Science and Linguistics).

For each benchmark instance:
1. It creates one HTML file per category (e.g., `T3_CS.html`) containing:
   - The original text
   - All tailored texts (for that category) generated by different LLMs/prompts
2. It generates three rotation variants (`_v1`, `_v2`, `_v3`) per category that reorder
   the explanations to reduce ordering bias in human evaluations.
3. Each explanation is labeled with a unique code name, following the pattern:
   `{target_category}_{model_letter}_{prompt_number}_{topic_number}` (e.g., `CS_g_2_3`).

Inputs
------
- `RANDOMIZED_BENCHMARK_PATH`: path to the JSON file with structured benchmark data.
- `TEXT_PAGES_PATH`: output directory where HTML files will be saved.
"""
...
for instance_code, data in benchmark_data.items():
    ...
    for target_category in ["CS", "L"]:  # CL excluded
        ...
        if not category_texts:
            print(f" Skipping {instance_code} - {target_category}: No texts found.")
            continue
